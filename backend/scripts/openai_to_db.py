...
from backend.database.queries.update_batch import update_batch
...
for i, item in enumerate(items):
    try:
        image = open(
            f"data/images/{item['outfit_id']}/{item['item_idx']}.jpg", "rb"
        ).read()
        ...
        print(
            f"Error processing item {item['outfit_id']}/{item['item_idx']}, Error: {e}"
        )
        continue

    except RateLimitError as e:
        print(
            f"Rate limit error for item {item['outfit_id']}/{item['item_idx']}, Error: {e}"
        )
        continue
